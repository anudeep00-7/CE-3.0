...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function performAdminLogin() {
        ...
        // Check mock admin users first (for demo purposes)
        const mockAdmin = usersData.find(u => u.username === username && u.password === password && u.role === 'admin');

        if (mockAdmin) {
            currentUser = mockAdmin;
            localStorage.setItem('currentUser', JSON.stringify(currentUser));
            showNotification(`Welcome, Administrator ${currentUser.username}!`, 'success');
            await loginSuccess();
            hideLoader();
            return;
        }
        ...
        if (auth) {
            try {
                ...
                if (userData.role !== 'admin') {
                    showNotification('Access denied. This account is not an administrator.', 'error');
                    await auth.signOut();
                    hideLoader();
                    return;
                }
                ...
                console.error('Firebase admin login error:', firebaseError.code, firebaseError.message);
            }
        }
